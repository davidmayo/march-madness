...
def create_bracket(bracket: Bracket | None = None) -> go.Figure:
    ...
    for game in bracket.games:
        ...
        within_round_indx = game.game_id - {
            0: 0,
            1: 32,
            2: 48,
            3: 56,
        ...
        y_base_position = - (y_index - games_this_round / 4 + 0.5) * y_spacing    # This works, somehow
        ...
        def get_text(team: Team | None, top_bottom: str, game_id: int | None = None) -> str:
            if game.game_id >= 32:
                return ""
            if team is None:
...
app.layout = html.Div(
    ...
    children=[
        html.Div(
            dcc.Graph(id='bracket', figure=bracket_figure, style={'height': '100%'}),  # Ensure the graph itself fills the height
            style={'flex': '1', 'padding': '10px', 'height': '100%'}  # Ensure the container fills the height
        ),
        html.Div(
            id='right',
